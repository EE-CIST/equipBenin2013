---
title: "L'EQUIPEMENT DES MENAGES DU BENIN EN 2013"
subtitle: "Devoir à la maison"
...
L'objectif du travail est d'apprendre à rédiger un chapitre de 2 à 4 pages en utilisant R markdown.

Le chapitre porte sur l'analyse d'une question du recensement du Bénin de 2013 relative à l'équipement des ménages composées de deux personnes mariées ou en union libre. Chaque étudiant s'est vu attribuer un équipement en cours. Le plan proposé est le suivant :


**1. Introduction**

- Présenter l'équipement choisi.
- Rappeler l'origine des données
- Annoncer le plan

**2. Résultats généraux**  

- Calculer le nombre général de ménages équipés et en déduire le taux d'équipement de la population du pays
- Commenter le résultat obtenu

**3. Inégalités spatiales**  
- Calculer le taux d'équipement des ménages par département
- Afficher le tableau correspondant
- Ajouter (au choix) une carte ou un graphique
- Commenter les résultats

**4. Inégalités sociales**
- Choisir une variable sociale (âge, sexe, propriétaire/locataire, niveau de diplôme) susceptible de créer des différences d'équipement
- Afficher le tableau d'équipement par catégorie sociale
- Ajouter  un graphique

- ommenter les résultats

**5. Partie libre (facultative)**
- Si vous avez assez de place, vous pouvez ajouter une analyse spatiale ou sociale de plus qui vous semble pertinente.

**6. Conclusion**
- Résumez en une ou deux phrases vos résultats principaux.

________________________________

N.B. Vous devrez remettre un dossier au format .zip contenant

- les fichiers de données dans un dossier "data"
- le programme R markdown
- les images externes utilisées pour illustrer la note (au format jpeg ou png) dans un sous-dossier "img"
- les figures produites pour illustrer la note au format .pdf dans un dossier "fig"
- une impression du chapitre dans le maximum de formats (.html, .doc ou .pdf)
...
Suite au cours, chaque étudiant s'est vu confié un équipement (tiré au hasard) pour lequel il devait rédiger une notice de 4 pages analysant à la fois les inégalités sociales et spatiales.
...
# Ouverture des données, format .csv
...
# Ajustements nécessaires pour le traitement des données
# Sélection de l'équipement
don$equip <- don$telfix

don$URBA<-as.factor(don$URBA)
levels(don$URBA) <- c("Rural","Urbain")

# Création de tranches d'âges
don$age4 <- cut(don$age, breaks = c(18,32,40,50,99))

# Sélection des variables qui seront étudiées
don<-don[,c("WGT","DEP_NAME","URBA","age4","sexe","etud5","equip")]

```

### 1. Le téléphone fixe : situation générale au Bénin 
:::: {style="display: flex;"}

::: {}
````{r, echo =FALSE, out.width="70%"}
knitr::include_graphics("img/wifix.png")
````
:::
...
Si le téléphone fixe tend à s'effacer depuis quelques années, il semble que le continent africain soit directement passé à la téléphonie mobile. Ainsi, une étude menée dans sept pays africains montraient qu'environ 1% des habitant-es possédaient une ligne fixe au Ghana, Nigeria ou encore Ouganda [@truta2015]. Cela s'explique notamment par le manque d'infrastructures nécessaires à son développement. En effet, la mise en route d'un service de téléphonie fixe nécessite l'installation de câbles spécifiques et l'existence d'un réseau de communication public. Ces services sont souvent très coûteux.Pour compléter cette étude, nous allons donc étudier la situation béninoise. 
:::

::::

#### 1.2. Le taux global d'équipement en téléphone fixe


Dans un premier temps, il convient d'analyser le niveau général d'équipement en téléphone fixe des ménages béninois[^1].

````{r, echo =FALSE}
# Mise en place du tableau pondéré pour calculer le taux d'équipement global du Bénin 
# en téléphone fixe
tab1 <- wtd.table(x=don$equip,
                  weights = don$WGT,
                  digits = 1)
tab1 <- freq(tab1)
# Utilisation du package "gt" car tab1 = data.frame
gt_tbl1 <- gt(tab1)
gt_tbl1 <-
  gt_tbl1 |>
  tab_header(
    title = "Table 1 : Taux d'équipement global en téléphone fixe",
  )
gt_tbl1
````

On constate rapidement que la population est très peu équipée en termes de téléphone fixe, car seuls **1,6%** des ménages comptabilisés possèdent un téléphone fixe. Le Bénin se trouve donc dans la moyenne, avec des chiffres similaires à d'autres pays africains comme le Ghana ou l'Ouganda. 

[^1]: On ne considère que les ménages composés d'au moins deux personnes vivant en couple.

### 2. Le téléphone fixe comme représentation des inégalités spatiales au Bénin ? 
````{r, echo =FALSE}
# Mise en place du tableau pondéré pour calculer le taux d'équipement par département 
# en téléphone fixe
tab2<-wtd.table(x=don$DEP_NAME,
                ...
                digits=3)
tab2<-lprop(tab2)
# Utilisation du package "kable"
knitr::kable(
  tab2,
  col.names = c("Département","Non","Oui","Total"),
  caption = "Répartition par département de l'équipement")
````
On constate assez rapidement une répartition assez égale de l'équipement en téléphone fixe dans les différents départements béninois. 


```{r, echo=FALSE,out.width="60%"}
tab2bis<-wtd.table(x=don$DEP_NAME,
                y=don$equip,
                weights=don$WGT,
                digits=0)
tab2<-lprop(tab2)

# Installation des packages de cartogaphie
...
# Ajout des données
map$equip_nb<-tab2bis[,2]
map$equip_pct<-lprop(tab2bis,total=F)[,2]
...
mf_layout(title = "Equipement du Bénin en téléphone fixe",
          credits = "Source : RP 2013, INS Bénin",
          scale = T, 
          frame=T )

```
Un département sort du lot avec un taux d'équipement relativement plus élévé : le département Littoral. Ce chiffre peut être expliqué par le fait que le département ne soit composé que de la capitale économique du pays, Cotonou.

Nous pouvons considérer les ménages vivant à Cotonou comme relativement plus aisés. En effet, dans la ville, l'incidence de la pauvreté monétaire était moins élevée, avoisinnant le 23,5% en 2015 contre 39,3% de moyenne nationale cette même année [@insae2020].

Ainsi, considérant cet élément, nous pouvons nous demander s'il n'existe pas un lien de corrélation, voire de causalité, entre la possession d'un téléphone fixe et la richesse d'un ménage.

### 3. Le téléphone fixe comme représentation des inégalités spatiales au Bénin ? 
#### 3.1. Le téléphone fixe, symbole de générations plus agées ? 
````{r, echo =FALSE}
# Mise en place du tableau pondéré pour calculer le taux d'équipement par tranche d'âge 
# en téléphone fixe
tab3 <- wtd.table(x=don$age4,
                  y=don$equip,
                  weights = don$WGT,
                  digits=3)
tab3 <- lprop(tab3)
knitr::kable(
  tab3,
  full_width=F,
  col.names = c("Classe d'âge","Non","Oui","Total"),
  caption = "Répartition par tranche d'âge du niveau d'étude",
  digits = 3)
````
Nous pouvons également penser que, comme dans d'autres régions du monde, le téléphone fixe est le symbole d'une génération plus agée. En effet, il semble que les plus jeunes soient aujourd'hui plus attiré-es par la plus grande offre de possibilités du téléphone mobile et délaissent le téléphone fixe. 

Ainsi, en étudiant le taux d'équipement en téléphone fixe des ménages en fonction de l'âge du chef de famille, on constate bien une faible augmentation de l'équipement quand le chef de famille a un âge compris entre **50 et 99 ans**. 

Dans un pays où la moitié de la population avait moins de 17 ans en 2021, il peut donc sembler logique que le taux d'équipement en téléphone fixe soit très bas [@DonneesMondiales2023].

#### 3.2. Ou simplement le symbole de foyers plus aisés ? 
:::: {style="display: flex;"}

::: {}
````{r, echo =FALSE}
# Mise en place du tableau pondéré pour calculer le taux d'équipement par niveau d'étude 
# en téléphone fixe
tab4 <- wtd.table(x=don$etud5,
                  y=don$equip,
                  weights = don$WGT,
                  digits=3)
tab4 <- lprop(tab4)
knitr::kable(
  tab4,
  full_width=T,
  col.names = c("Niveau d'éducation","Non","Oui","Total"),
  caption = "Répartition par niveau d'étude de l'équipement",
  digits = 3)
````
:::
...
plot(tab4bis,
     main="Figure 1: Répartition de l'équipement en téléphone
     fixe en fonction de l'éducation des ménages",
     sub="Source : RP 2013, INS Bénin",
     col=c("aliceblue","cornsilk"))
```` 
:::

::::

Comme nous avons commencé à le constater plus haut en étudiant la répartition de l'équipement en téléphone fixe dans les différents départements béninois, il semble bien que les ménages les plus aisés aient plus tendance à posséder un téléphone fixe. 

N'ayant pas de données précises sur les revenus moyens des ménages ici, nous considérerons les ménages les plus aisés comme les ménages ayant fait le plus d'études. 

Suivant ce principe, nous pouvons constater que le taux d'équipement en téléphone fixe tend effectivement à augmenter à mesure que la durée d'étude augmente. Ainsi, **6,7%** des ménages ayant réalisé des études supérieures ont un téléphone fixe, contre **1,2%** pour les ménages qui n'ont pas réalisé d'études.

Cependant, ce taux reste très faible et l'immense majorité des ménages sondés ne possèdent pas de téléphone fixe, comme nous pouvons le constater sur ce graphique.


### 4. Conclusion générale
Nous pouvons donc conclure que l'étude du taux d'équipement en téléphone fixe des ménages montre un large sous-équipement. Surtout quand on compare ce taux avec le taux d'équipement américain, ou plus de 60% des foyers déclarent un téléphone fixe.

Cependant, ce taux d'équipement en téléphone fixe peut tout de même être considéré comme représentatif d'inégalités socio-spatiales. Il illustre aussi les grands enjeux du Bénin : une population très jeune, dont la pauvreté est influencée par le lieu de vie ; une zone rurale ou une zone urbaine. 

Enfin, nous pouvons penser qu'une actualisation de ces chiffres ne montrerait qu'une baisse du taux d'équipement, du fait de la victoire sans appel du téléphone mobile pour l'usage des particuliers, partout autour du globe. 
...
```{r}
knitr::opts_chunk$set(echo = TRUE) #options générales : code invisible
...
L'objectif de ce cours est d'apprendre aux étudiants l'usage de R markdown en prenant l'exemple de l'exploitation du recensement général de population de 2013 au Bénin. 
...
On regroupe ici les références collectées par les étudiants pour commenter leurs différents chapitres (à condition qu'ils aient utilisé le fichier .bib)
